...
class BodyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        draftTextView.layer.borderColor = Helper.orangeColor.cgColor
        ...
        if let parentId = parentId {
            ...
            do{
                ...
                loadIdeas()
                if(essay?.body != "" && essay?.body != nil){
                    bodyTextView.text = essay?.body
                }
                if(essay?.bodyDraft != "" && essay?.bodyDraft != nil){
                    draftTextView.text = essay?.bodyDraft
                }
            }catch{
                print(error)
            }
    ...
    @objc func backAction(sender: UIBarButtonItem) {
        if(essay?.body != bodyTextView.text || essay?.body != textView.text || essay?.bodyDraft != draftTextView.text){
            ...
            refreshAlert.addAction(UIAlertAction(title: "Yes", style: .cancel, handler: { (action: UIAlertAction!) in
                self.essay?.body = self.bodyTextView.text
                self.essay?.bodyDraft = self.draftTextView.text
                self.saveItems()
                self.isDismissed?()
    ...
    @IBAction func saveBtnPressed(_ sender: UIButton) {
        essay?.body = bodyTextView.text
        essay?.bodyDraft = draftTextView.text
        saveItems()
        isDismissed?()
        navigationController?.popViewController(animated: true)
...
extension BodyViewController: UITableViewDataSource{
